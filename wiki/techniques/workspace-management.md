...
Workspace management is the connective tissue between technique-tiling-strategy (which chooses the shapes) and technique-double-buffering (which multiplies their cost). Concrete uses appear in the matmul and attention kernels of this knowledge base — for example, split-K GM partials in matmul-ascendc and per-row reduction scratch in layernorm-style kernels. The exact UB capacity is hardware-specific (consult wiki-hardware-unified-buffer for per-architecture figures); treat the 192KB–256KB range as the 910B-class working assumption, not a universal constant.
